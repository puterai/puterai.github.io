(function() {
    ...
    const geminiRefreshBtn = document.getElementById('geminiRefreshBtn');
    ...
    const checkFlash = document.getElementById('checkFlash');
    const checkPro = document.getElementById('checkPro');
    const flashItem = document.querySelector('.dropdown-item[data-model="google/gemini-3-flash-preview"]');
    const proItem = document.querySelector('.dropdown-item[data-model="google/gemini-3.1-pro-preview"]');

    let currentModelId = 'google/gemini-3-flash-preview';
    ...
    function createModelTag(modelId, elapsedSeconds = null) {
        if (modelId === 'google/gemini-3.1-pro-preview' && elapsedSeconds !== null) {
    ...
    function updateDropdownSelection(modelId) {
        checkFlash.style.display = modelId === 'google/gemini-3-flash-preview' ? 'flex' : 'none';
        checkPro.style.display = modelId === 'google/gemini-3.1-pro-preview' ? 'flex' : 'none';
        selectedLabelSpan.textContent = modelId === 'google/gemini-3-flash-preview' ? '快速' : 'Pro';
    }
    ...
    flashItem.addEventListener('click', () => {
        currentModelId = 'google/gemini-3-flash-preview';
        updateDropdownSelection(currentModelId);
        dropdown.classList.remove('active');
    });
    proItem.addEventListener('click', () => {
        currentModelId = 'google/gemini-3.1-pro-preview';
        updateDropdownSelection(currentModelId);
        dropdown.classList.remove('active');
    });
    ...
    function createCopySVG() {
        ...
        path1.setAttribute("fill", "#4a5e7a");
        ...
        path2.setAttribute("fill", "#4a5e7a");
    ...
    function addMessage(text, isUser = false) {
        ...
        if (isUser) {
            // 修复点：对用户的原生输入执行 HTML 实体字符编码转义，防止直接执行渲染
    ...
    geminiRefreshBtn.addEventListener('click', () => location.reload());
